#! /usr/bin/env python3
"""
    This is a class to handle the SemanticKITTI dataset.
"""
...
class SemKitti_Handler:
    def __init__(self, path_to_dataset:str, 
                       config:dict,
                       verbose:bool=False):
        """
            This is a class to handle the SemanticKITTI dataset, containing any number of sequences.
            Args:
                path_to_dataset: str, path to the SemanticKITTI dataset
            ...
            Note:
                ...
                |── sequences
                    ├── 00
                    │   ├── depth_map (contains the range images, not used here)
                    │   │   |── depth
                    │   │   │   ├── 000000.png
                    │   │   |   ├── ...
    ...
    def load_kitti(self, sequence:str):
        """ Read the SemanticKITTI dataset and return the pointclouds, poses, depth maps, and calibration data.
        ...
        ## Load the labels
        labels, instances = self.load_labels(self.path_to_dataset + f'/sequences/{sequence}/labels/')
        ...
        return poses, scans, T_cam_velo, labels, instances

    ## Load the SemanticKITTI calibration data.
    ...
    ## Load the SemanticKITTI timestamps.
    ...
    ## Load the SemanticKITTI ground truth poses.
    ...
    ## Load the SemanticKITTI depth maps.
    def load_depth_map(self, depth_map_path:str) -> List[np.ndarray]:
        ...
        for depth_map_file in np.sort(os.listdir(depth_map_path)):
    ...
    ## Load the SemanticKITTI point cloud scans.
    def load_scans(self, pointcloud_path:str) -> List:
        """ Load the pointclouds from the SemanticKITTI dataset. The format is .bin files.
            Args:
    ...
    ## Load labelrs that include class and instance information.
    def load_labels(self, label_path:str) -> Tuple[List, List]:
        """Load the labels from the SemanticKITTI dataset. The format is .label files.
            Args: 
                label_path: The path to the label folder
            Returns:
                labels: A nxmx1 numpy array with n labels and m points with class 
                instances: A nxmx1 numpy array with n labels and m points with instance
        """
        ...
        for label_file in label_files:
            curr_label = np.fromfile(os.path.join(label_path, label_file), dtype=np.uint32)
            curr_instance = curr_label >> 16
            curr_label = curr_label & 0xFFFF
            remap_label = np.zeros_like(curr_label)
            for key in self.config['learning_map']:
                remap_label[curr_label == key] = self.config['learning_map'][key]
            inv_remapped_label = np.zeros_like(remap_label)
            for key in self.config['learning_map_inv']:
                inv_remapped_label[remap_label == key] = self.config['learning_map_inv'][key]
            ## Append the label to the list
            labels.append(inv_remapped_label)
    ...
    ## Visulize the SemanticKITTI ground truth poses.
    def visualize_poses(self, poses:np.ndarray, poses2:np.ndarray=None) -> None:
        ...
        ax.set_ylabel('Y [m]')
        ## Set the title
        ax.set_title('SemanticKITTI ground truth poses')
        ## Add grid
    ...
    ## Visualize a SemanticKITTI scan.
    def plot_scan(self, scan:np.ndarray) -> None:
        """ Visualize a SemanticKITTI scan using matplotlib.
            Args:
        ...
        ax.set_ylabel('Y [m]')
        ## Set the title
        ax.set_title('SemanticKITTI scan')
        ## Add grid
        plt.grid(alpha=0.5, linestyle='--', linewidth=1.0)
        ## Add limit
        plt.xlim(-100, 100)
